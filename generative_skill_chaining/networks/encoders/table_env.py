...
from generative_skill_chaining import envs
from generative_skill_chaining.networks.encoders import Encoder
from generative_skill_chaining.utils import tensors
...
class TableEnvEncoder(Encoder):
    ...
    @staticmethod
    def derearrange_observation(
        observation: torch.Tensor,
        policy_args: Union[np.ndarray, Dict[str, List[int]]],
        randomize: bool = True,
        ):

        dummy_observation_indices = TableEnvEncoder._get_observation_indices(
            policy_args, randomize=randomize
        )

        observation_indices = np.zeros_like(dummy_observation_indices)

        for i in range(len(observation_indices)):
            observation_indices[i] = np.where(dummy_observation_indices == i)[0][0]
    ...
    def backward(
        ...
        **kwargs,
        ...
        observation = torch.reshape(observation, (-1, 8, 12))

        observation = (observation * self.observation_range) + self.observation_mid

        observation = TableEnvEncoder.derearrange_observation(
            observation, policy_args, randomize
        )

        return observation

    def unnormalize(self, observation: torch.Tensor) -> torch.Tensor:
        """Un-normalizes the observation to the original range."""
        return (observation * self.observation_range) + self.observation_mid
