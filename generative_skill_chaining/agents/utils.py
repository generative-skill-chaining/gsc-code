...
import generative_skill_chaining
from generative_skill_chaining import agents, envs, encoders
from generative_skill_chaining.utils import configs
...
class AgentFactory(configs.Factory):
    ...
    def __init__(
        ...
        if checkpoint is not None:
            ...
            print(f"[research] Loading agent from config {config}")
...
def get_env(env_class, env_kwargs, wrapper, wrapper_kwargs):
    ...
    if wrapper is not None:
        env = vars(generative_skill_chaining.envs)[wrapper](env, **wrapper_kwargs)
    return env
...
def get_env_from_config(config):
    ...
    env_class = configs.parse_class(config, "env", generative_skill_chaining.envs)
...
def get_model(
    ...
    alg_class = vars(generative_skill_chaining.agents)[config["alg"]]
    dataset_class = (
        None
        if config["dataset"] is None
        else vars(generative_skill_chaining.datasets)[config["dataset"]]
    )
    network_class = (
        None
        if config["network"] is None
        else vars(generative_skill_chaining.networks)[config["network"]]
    )
    optim_class = (
    ...
    processor_class = (
        ...
        else vars(generative_skill_chaining.processors)[config["processor"]]
    )
...
#     model.path = path
#     assert issubclass(type(model), generative_skill_chaining.agents.Agent)
#     schedule = (
