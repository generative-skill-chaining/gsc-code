...
from generative_skill_chaining.mixed_diffusion.datasets_transformer import get_primitive_loader
...
def train_model(
    ...
    for epoch in tqdm.tqdm(range(num_epochs)):
        ...
        for batch in dataloader_transition:
            ...
            obs_ind = obs_ind.to(device).float()
            loss = diffusion_model_transition.get_loss(data, obs_ind)
        ...
        if (epoch + 1) % 10 == 0:
            if path is not None:
                torch.save(diffusion_model_transition.state_dict(), path / "diffusion_model_transition.pt")
    if path is not None:
        torch.save(diffusion_model_transition.state_dict(), path / "diffusion_model_transition.pt")
